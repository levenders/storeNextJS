  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className={cn(styles.reviewForm, className)} {...props}>
        ...
        <Input
          {...register('title', {
            required: { value: true, message: 'Заголовок обязателен' },
          })}
        ...
        <div className={styles.submit}>
          <Button appearance="primery" tabIndex={isOpened ? 0 : -1}>
            Отправить
          </Button>
      ...
      {isSuccess && (
        <div className={styles.success}>
          ...
          <span className={styles.icon} onClick={() => setIsSuccess(false)} />
        </div>
      )}
      {error && (
        <div className={styles.error}>
          ...
          <div>{error}</div>
          <span className={styles.icon} onClick={() => setError(undefined)} />
        </div>
      )}
    </form>
  )
